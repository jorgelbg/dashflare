...
describe('request handler', () => {
  ...
  it('sends payload to storage', async () => {
    const headers: HeadersInit = new Headers({
      ...
      referer: 'https://github.com/jorgelbg/dashflare',
    })

    const event = new FetchEvent('fetch', {
      request: new Request('https://github.test.workers.dev/', {
        headers,
      }),
    })
    ...
    expect(bodyObj.streams[0].entries[0].line).to.satisfy((string) =>
      [
        ...
        'owner=jorgelbg',
        'repo=dashflare',
        'url=https://github.test.workers.dev/',
    ...
    expect(bodyObj.streams[0].labels).to.satisfy((string) =>
      ['device_type="desktop"', 'method="GET"', 'status="200"'].every((bit) =>
        string.includes(bit),
      ),
    )
